      ...
      <div className="mt-4 pl-0 sm:pl-9">
        <div className="rounded-2xl border bg-card/60 p-4 shadow-sm sm:p-5">
          ...
          <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-muted">
            <div
              className="progress-shimmer h-full rounded-full bg-primary transition-[width] duration-700 ease-out"
...
export function ResearchWorkbench() {
  ...
  useEffect(() => {
    if (run?.status === "completed") {
      // Completed process work intentionally disappears from the conversation.
      // The user is left with the research result rather than implementation noise.
    ...
    <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-4xl flex-col sm:min-h-[calc(100vh-5rem)]">
      ...
      <div className="sticky bottom-0 z-20 -mx-2 bg-gradient-to-t from-background via-background/98 to-transparent px-2 pb-3 pt-8 sm:pb-5">
        ...
        <ChatComposer
          ...
          onFileChange={setFile}
          onSubmit={() => {
            if (run?.status === "completed" || run?.status === "failed") startFreshResearch()
            // startFreshResearch is synchronous; submit on the next frame so state settles.
